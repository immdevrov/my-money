...
export const MUTATIONS = [
  ...
  ['M176 month window is 6 periods', COMPARE,
    '{ month: 6, quarter: 4, year: 3 }', '{ month: 12, quarter: 4, year: 3 }'],
  ['M177 quarter window is 4 periods', COMPARE,
    '{ month: 6, quarter: 4, year: 3 }', '{ month: 6, quarter: 5, year: 3 }'],
  ['M178 year window is 3 periods', COMPARE,
    '{ month: 6, quarter: 4, year: 3 }', '{ month: 6, quarter: 4, year: 4 }'],
  ['M179 year ago is one year back', PERIOD,
    'Number(period.slice(0, 4)) - 1}', 'Number(period.slice(0, 4)) - 2}'],
  ['M180 year ago must lie inside the span', COMPARE,
    'const yearAgo = withYearAgo && inSpan.has(yearAgoCandidate) ? yearAgoCandidate : null;',
    'const yearAgo = withYearAgo ? yearAgoCandidate : null;'],
  ['M181 years have no year-ago column', DASHVIEW,
    "if (periodType !== 'year') {", 'if (true) {'],
  ['M182 missing-rate count covers the year-ago period', COMPARE,
    '...(yearAgo === null ? [] : [yearAgo])', '...([] as string[])'],
  ['M183 previous header names the period type', DASHVIEW,
    'label: `Previous ${noun}`', 'label: `Previous period`'],
  ['M184 window header keeps N', DASHVIEW,
    'const window = `${WINDOW_SIZE[periodType]} ${noun}s`;',
    'const window = `${Math.min(WINDOW_SIZE[periodType], (view?.options.length ?? 1) - 1)} ${noun}s`;'],
];
